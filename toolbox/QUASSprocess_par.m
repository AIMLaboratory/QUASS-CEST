% Huabin Zhang - huabinz@connect.hku.hk, 20250310
% QUASS process for zspec
%   ref: Sun Zhe-2011. MRM 86(2): 765-776.

function [zspecQUASSList, R1rhoList] = QUASSprocess_par(zspecList, offs_ppm, SATpara, R1w_vec, db0map_vec,display_on)
% INPUT: 
%   zspecList: [nf,nzspec]
%   offs_ppm: [1, nf]
...
%   db0map_vec: [1, nzspec], in ppm
    ...
    parfor idxZ = 1:nzspec
        ...
        % temporary variable for current loop
        temp_R1rhoList = zeros(nf, 1);
        ...
        'lb',R1rho_lb,'ub',R1rho_ub,'xdata',offs_ppm(idxoffs),'ydata',zspec(idxoffs),'options',OPTIONS);
            ...
            temp_R1rhoList(idxoffs) = par(1);
        end
        
        theta_List = atan(B1./B0./ (offs_ppm-db0map_vec(idxZ) ) ); 
        temp_zspecQUASSList = (R1w./reshape(temp_R1rhoList,[],1)) .* cos(theta_List).^2;
        
        % save temporary variable
        R1rhoList(:, idxZ) = temp_R1rhoList;
        zspecQUASSList(:, idxZ) = temp_zspecQUASSList;
    end
