...
B1List = linspace(0.3, 1.5, 26);
TsList = [10,linspace(0.5, 3.0, 24)];
outputfolder = ".\nB1-nTs";
...
nTs = length(TsList);
TdList = TsList;
save(outputfolder+"\B1_Ts_List.mat",'B1List','TsList','TdList');
...
for idxB1 = 1:nB1
    ...
    for idxTs = 1:nTs
        ...
        %% simulation with M0
        [zspecList,paraList] = func1_dataSim_givenSATpara(SATpara, offs, Npixel); 
        ...
        for idxalgo = 1:length(fitting_methods)
            ...
            for idxprepro = 1:2 % QUASS
                ...
                diffss_meanList_all_givenB1(1,idxalgo+(idxprepro-1)*3,idxTs) = ...
                    mean(abs(squeeze(ssContrast_all(1,idxalgo+(idxprepro-1)*3,:)) - Zamide_DZ_vec(:)),'all');
                diffss_meanList_all_givenB1(2,idxalgo+(idxprepro-1)*3,idxTs) = ...
                    mean(abs(squeeze(ssContrast_all(2,idxalgo+(idxprepro-1)*3,:)) - Zguan_DZ_vec(:)),'all');
                diffss_meanList_all_givenB1(3,idxalgo+(idxprepro-1)*3,idxTs) = ...
                    mean(abs(squeeze(ssContrast_all(3,idxalgo+(idxprepro-1)*3,:)) - Znoe_DZ_vec(:)),'all');
    ...
    %% save data
    save(outputfolder+"\simData_B1_"+num2str(B1List(idxB1))+".mat",...
