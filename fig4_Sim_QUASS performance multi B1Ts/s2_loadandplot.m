...
%% load data
load(dataFolder+"B1_Ts_List.mat",'B1List','TsList');
% B1List(4:end)=[];
% B1List(end-2:end) = [];
nB1 = length(B1List);
nTs = length(TsList);
...
% sort file
fileList = dir(dataFolder+'simData_B1_*.mat');
...
diffArr_all = diffArr_all(:,:,idxTsList,:);
TsList(end) = [];
R2Arr_all(:,:,end,:) = [];
sensArr_all(:,:,end,:) = [];
diffArr_all(:,:,end,:) = [];
...
y_ticks_show = [0.5,1.0,1.5,2.0,2.5];
...
Fig3 = figure();set(gcf,'Position',[50 50 1200 850]);
tiledlayout(3,6,"TileSpacing","loose","Padding","loose")
colormap parula; 
...
exportgraphics(Fig2, OutputDir+"fig4_mB1_mTs_R2.png", 'BackgroundColor', 'white', 'Resolution', resol);
exportgraphics(Fig3, OutputDir+"fig4_mB1_mTs_diff.png", 'BackgroundColor', 'white', 'Resolution', resol);
