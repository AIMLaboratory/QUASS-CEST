...
% dateDirList = ["D:\InvivoData\GE\MS\MotionCorr(8MS8HC)\ms\20250522\"];

outputFolder = ".\preprocessedData_multi_batch\";
...
for fileidx = 1:length(dateDirList)
    ...
    load(dataDir+"Apro\2D_spm_preproc.mat","T1w","T2w","roi_gm_uint","roi_wm_uint","roi_csf_uint",...
    ...
    % motion correction
    if motioncor_flag == 1
        CEST_data(1:3) = motionCorCEST(CEST_data(1:3),outputFolder+"fig_motioncor_B1_0.5_"+dataID+".png"); % revise field 'raw_re'
        CEST_data(4:6) = motionCorCEST(CEST_data(4:6),outputFolder+"fig_motioncor_B1_1.0_"+dataID+".png"); % revise field 'raw_re'
    else
    ...
    nexttile;imshow(T1w,[]);title('T1w')
    nexttile;imshow(CEST_data(1).m0,[]);title('CEST@M0')
    nexttile;imshow(roi,[]);title('roi')

    nexttile;imshow(roi_gm);title('roi gm')
    nexttile;imshow(roi_wm);title('roi wm')
    nexttile;imshow(roi_csf);title('roi csf')

    ax = nexttile;imagesc(T1map);axis off;clim([0,4]);colorbar;colormap(ax,jet);title('T1map [s]')
    nexttile;imagesc(WASABI_data.B1map./3.7*100);axis off;colorbar;title('B1map [a.u.]')
    nexttile;imagesc(WASABI_data.db0map);axis off;colorbar;title('db0map [ppm]')

    %% save the preprocessed data
    fprintf('\tsave to .mat file\n')
    save(outputFolder+"QUASS_preproc_"+dataID+".mat", ...
                "T1w","T1map","T2w","roi_csf","roi_gm","roi_wm","roi", ...
                "offs_WASABI","offs_CEST", ...
                "WASABI_data","CEST_data");
    exportgraphics(Fig, outputFolder+"fig_raw_"+dataID+".png", 'BackgroundColor', 'white', 'Resolution', 600);
end
...
function CEST_data = motionCorCEST(CEST_data,figdir)
% INPUT:
