...
% exportgraphics(Fig, filefolder+"\Sat#"+num2str(idxSat)+"_sublesion.png", 'BackgroundColor', 'white', 'Resolution', 600);
...
%   [GM/WM]_[HC/MS/lesion]: Nx1 cell double array
    ...
    [p_anova_GM, ~, stats_GM] = anova1([GM_HC; GM_MS], ...
        [repmat({'A2'}, length(GM_HC), 1); 
         repmat({'B2'}, length(GM_MS), 1)], 'off');
    % fprintf('ANOVA p-value for GM measurements = %.4f\n', p_anova_GM);
    
    [c_GM, ~, ~, ~] = multcompare(stats_GM, 'CType', 'tukey-kramer','display','off');
    % disp('Tukey HSD results for GM measurements:');
    % disp(array2table(c_GM, 'VariableNames', ...
    %     {'Group1', 'Group2', 'LowerCI', 'Estimate', 'UpperCI', 'p'}));
    
    % Three WM measurements (HC, MS, MS lesion)
    [p_anova_WM, ~, stats_WM] = anova1([WM_HC; WM_MS; WM_lesion], ...
        [repmat({'A1'}, length(WM_HC), 1); 
         repmat({'B1'}, length(WM_MS), 1); 
         repmat({'C1'}, length(WM_lesion), 1)], 'off');
    % fprintf('ANOVA p-value for WM measurements = %.4f\n', p_anova_WM);
    
    [c_WM, ~, ~, ~] = multcompare(stats_WM, 'CType', 'tukey-kramer','display','off');
    % disp('Tukey HSD results for WM measurements:');
    % disp(array2table(c_WM, 'VariableNames', ...
    %     {'Group1', 'Group2', 'LowerCI', 'Estimate', 'UpperCI', 'p'}));
